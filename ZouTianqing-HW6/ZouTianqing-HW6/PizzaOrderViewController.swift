...
class PizzaOrderViewController: UIViewController {
    ...
    @IBAction func pressedDone(_ sender: Any) {
        ...
        // Save pizza to Firestore first, so we get the document id for later deletion.
        let pizza = Pizza(
            size: selectedSize,
            crust: selectedCrust!,
        ...
        // Create the document reference first so we can safely use `ref.documentID` inside the callback.
        let ref = db.collection("pizza").document()
        ref.setData(pizza.toFirestoreData()) { [weak self] error in
            guard let self else { return }
            if let error = error {
                print("Error saving pizza: \(error)")
                DispatchQueue.main.async {
                    self.summaryLabel.text = "Failed to save pizza."
                }
                return
            }
            
            // Attach the real Firestore document id to enable swipe-to-delete.
            pizza.documentID = ref.documentID
            
            DispatchQueue.main.async {
                self.delegate?.didCreatePizza(pizza)
                self.summaryLabel.text =
                """
                One \(self.selectedSize.lowercased()) pizza with:
                    \(self.selectedCrust!)
                    \(self.selectedCheese!)
                    \(self.selectedMeat!)
                    \(self.selectedVeggies!)
                """
            }
        }
    }
